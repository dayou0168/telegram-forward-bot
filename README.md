...
- 机器人入群自动登记群组；收到群内消息时也会自动登记，重装后可在群里发送 `/register` 主动登记当前群。
- 宿主由服务器环境变量固定指定。
- 宿主可以添加、禁用操作人。
- 宿主可以给每个操作人分配可访问分组；操作人只能看到、管理、发送到已授权分组。
- 宿主可以分别开关操作人的“分组群发”和“单群发送”权限，并给操作人单独勾选可单群发送的群。
- 操作人本人不需要加入目标群；授权后可以通过机器人向群内发送，并通过回复通知里的「快速回复」和群内沟通。
- 宿主可以设置直属操作人是否允许继续添加下级操作人；下级操作人不能继续添加操作人。
- 宿主可以给操作人单独开启“看发送”和“看回复”，允许其看到其他操作人通过机器人发送的内容和群内回复通知。
- 操作人通过机器人发送到群里的内容会同步通知宿主和开启“看发送”的操作人；群内回复通知也会发给宿主、原发送操作人和开启“看回复”的操作人。
- 宿主可以给每个操作人单独设置私聊自动清空时间，按北京时间每天执行；只清空该操作人与机器人的私聊记录，不会删除宿主与机器人的聊天记录。
- 宿主开启“设置下级”后，直属操作人可以再创建一层下级操作人，并只能把自己有权限的分组授权给下级；下级操作人不能继续创建操作人。
- 操作人可以创建、删除、重命名分组。
- 操作人可以给分组添加或删除指定群组。
- 操作人添加群组时，只能看到自己已经加入的群组；宿主可以看到全部已登记群组。
- 宿主和操作人可以选择分组，确认后群发消息。
- 宿主和有单群权限的操作人可以切换到“指定群发送”，把消息只投递到一个授权群。
- 非宿主、非操作人的私聊和按钮操作无效。
...
```text
ghcr.io/dayou0168/telegram-forward-bot:0.2.9
```
...
Telegram Bot API 不能主动列出机器人已经加入的所有群。重新安装如果换了新数据库或删除了 Docker 数据卷，机器人之前登记过的群就看不到了。

可用的恢复方式：

- 保留或恢复旧数据库，这是最完整的方式。
- 在 BotFather 里关闭 Group Privacy，让机器人能收到群内普通消息。之后群里只要有人发消息，机器人就会自动登记这个群。
- 对完全没人说话的老群，在群里发送 `/register`，机器人会立即登记当前群。

BotFather 关闭 Group Privacy：
...
关闭后，已经有机器人的群里只要有人发一条消息，机器人就会自动重新登记。注意：如果群一直没有新消息，机器人仍然无法“凭空知道”这个群，需要发 `/register` 或恢复旧数据库。

宝塔部署时，数据库在 Docker 命名卷 `tg_forward_notice_data` 里。重建/重启容器不会丢数据，但删除这个数据卷会丢失分组、操作人和群组登记记录。
...
4. 在「权限管理」里点击某个操作人，可以开关“分组群发 / 单群发送 / 设置下级”，并分别进入「分组权限」或「单群权限」勾选授权范围。
5. 进入「分组管理」创建分组。
6. 进入某个分组，点击「添加群组」或「批量添加」把已登记的群加入分组。
7. 点击「发送消息」，选择分组发送，或切换到指定群发送。
...
宿主可以在「权限管理」进入某个操作人详情，点击「清空」单独设置这个操作人的清空时间。时间按北京时间保存，支持 `00:00`、`0:00`、`0.00` 这类输入；发送「关闭」可以停用该操作人的自动清空。

清空只针对开启后这个操作人与机器人私聊窗口中已经记录到的消息，包括操作人发给机器人的消息，以及机器人发给这个操作人的菜单、提示和通知。宿主与机器人的私聊记录不会被删除。实际删除仍受 Telegram Bot API 限制；过旧、已不存在或 Telegram 不允许删除的消息会被跳过。数据库只保存待清空队列，正常到点后会清掉；超过 3 天的待删记录也会自动丢弃，避免长期累积。
...
机器人会在有对应固定内容时尝试编辑群里被回复的那条机器人投递消息，但不会删除群成员发出的回复消息。宿主可以在主菜单进入「机器人配置」，分别设置固定替换文字和固定替换图片；只有设置固定文字后，文字原消息才会替换成固定文字；图片原消息会替换成固定图片并保留原 caption；图片+caption 若没有设置固定图片但设置了固定文字，则只替换 caption。
